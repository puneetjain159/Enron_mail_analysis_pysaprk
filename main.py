...
DIR = "/Users/puneet.jain/Downloads/maildir/ring-a"
...
data = spark.read.format('parquet').options(headers = True).load('Downloads/enron_correct')
...
data_join_df = data_join_df.where(col("diff_secs") >= 0)

mail = data_join_df.where(col("diff_secs") == data_join_df.agg({"diff_secs": "min"}).collect()[0][0]).collect()[0]
